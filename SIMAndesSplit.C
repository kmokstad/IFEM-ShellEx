...
{
  if (time.it == 0)
    newLHS = true; // Need to rebuilt LHS due to change in material properties

  if (myRegions[1].myElements.empty()) // Only one material region
    return this->SIMAndesShell::assembleSystem(time,prevSol,newLHS,poorConvg);

#ifdef INT_DEBUG
  std::cout <<"Splitting the domain into two material regions: "
  ...
  for (Region& material : myRegions)
  {
    ...
    for (ASMbase* pch : myModel) // Activate elements in this meterial region
      pch->setActiveElements(&material.myElements);
