                              ...
                              LocalIntegral& elmInt)
{
...
#ifdef USE_OPENMP
  double& rho = rhoPt[omp_get_thread_num()];
  double& thk = thkPt[omp_get_thread_num()];
#else
  double& rho = rhoPt.front();
  double& thk = thkPt.front();
#endif
  ...
  {
    double dum1, dum2;
    ok = currentPatch->getProps(fe.iel,fe.idx,dum1,dum2,rho,thk);
    if (ovrMat) rho = Rho; // Override the patch-level material properties
  }
  ...
  if (thickRi+trOutside > 0.0)
    thk *= 1.0 - (Xc.inside(Xlow,Xupp) ? thickRi : trOutside);
...
#else
  const double rhoThk = rhoT.front()*thkPt.front();
#endif
                                  ...
                                  const TimeDomain& time, size_t)
{
  ...
  if (nenod >= 3)
  {
    if (!ovrMat && !currentPatch->getProps(fe.iel,fe.idx,E,nu,rho,thk))
      return false;
...
#ifdef HAS_ANDES
    double E = Emod, nu = Nu, rho = Rho, thk = Thck0;
    if (currentPatch && !currentPatch->getProps(fe.iel,fe.idx,E,nu,rho,thk))
      return false;
#ifdef USE_OPENMP
    rho = rhoPt[omp_get_thread_num()];
    thk = thkPt[omp_get_thread_num()];
